...
* 22.05.2015 BIOS/booting
  ...
  MBR (Dos Label) format: 512b. Of them, after 2 bytes is 0x55AA. That's for the initial check that bus works. +0x55AA guarantees that it's MBR. Basically, MBR loads some memory block and then launches it. First 510 bytes of MBR is jump, metadata, loader and TBL. Jump skips TBL. TBL contains boot bit, type, start and length. Metadata is needed after the OS loaded. TLB size is 4 partitions, but it can be extended with some костыль. TLB is made this way just because Windows doesn't accept existence of another operating systems.
